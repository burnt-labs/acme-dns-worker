...
const cleanupRoute = createRoute({
  ...
  description:
    "Deletes the `_acme-challenge.<subdomain>` TXT record owned by the calling " +
    "vendor. Records belonging to other vendors are never touched. Removing a " +
    "record that does not exist is a success, so cleanup hooks are idempotent.",
  security: [{ ApiKeyAuth: [] }],
  request: {
    body: {
      required: true,
...
cleanupRoutes.openapi(cleanupRoute, async (c) => {
  ...
  let deleted: boolean;
  try {
    deleted = await dns.deleteAcmeChallenge(subdomain, vendor);
  } catch (err) {
  ...
  console.log(
    `TXT record cleanup: domain=${subdomain} vendor=${vendor} deleted=${deleted}`,
  );
  return c.json({ deleted, vendor }, 200);
});
